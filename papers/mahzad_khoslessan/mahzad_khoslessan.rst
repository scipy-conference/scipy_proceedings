...
Depending on the file format the loading time of frames into memory will be different depending on the file format.
...
Figures :ref:`IO-comparison` compare the difference in I/O time for different file formats for 300X trajectory for multiprocessing and distributed schedulers respectively. 
...
As a result, opening the same file again is fast (Figure :ref:`IO-multiprocessing` B and :ref:`IO-distributed` B). 
...
This is why IO time per frame for XTC file format remains level with increasing the nember of processes for both schedulers (Figure :ref:`IO-comparison` B, E).
...
This is since NCDF has a more complicated file format. 
...
As described above, IO time per frame remains pretty level for DCD file format using SSDs (Figure :ref:`IO-comparison` A).
As a result DCD file formats using SSDs show a very good scaling on a single node.
Moreover, job execution time for DCD file format using SSDs is one order of magnitude smaller than other formats.
Based on Figure :ref:`time-comparison` A, B, C, reducing IO time has lead to noticeable improvement in performance which emphasizes the impact of IO time on the overall performance.
