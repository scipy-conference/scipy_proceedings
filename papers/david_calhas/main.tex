...
Ultimately, the goal of this package is to collect, in one package, methods for EEG to fMRI synthesis. We would welcome contributions from authors of related work such as \cite{calhas2022eeg} and \cite{liu2019convolutional}. In the future, we plan to add a module or example folder with implementations of these approaches, so that other research groups can easily access them and reproduce key results. 
