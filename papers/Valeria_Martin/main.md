...
abstract: |
Forest loss due to natural events, such as forest wildfires presents an increasing global challenge that demands advanced methods for effective and accurate detection and mitigation. To this end, integrating satellite imagery with deep learning (DL) methods has become essential, but this approach requires substantial labeled data. This study introduces the California Wildfire GeoImaging Dataset (CWGID), a high-resolution satellite imagery dataset developed to enhance forest wildfire detection through advanced machine learning. Created and processed using Python and its Google Earth Engine (GEE) API, the CWGID comprises over 100,000 labeled 'before' and 'after' wildfire image pairs for use in deep-learning applications. An analysis of the dataset using Convolutional Neural Network (CNN) architectures, such as VGG16 and EfficientNet, for detecting forest wildfires, achieved accuracies of respectively 82% and 93%.The python-based methodology used to build the dataset, highlights Python's critical role in the seamless integration of satellite imagery with DL methods, providing a framework for improving  forest change detection. 
---

## Introduction

This paper outlines the Python methodology used for generating and curating the California Wildfire GeoImaging Dataset (CWGID), a Sentinel-2 Satellite Imagery dataset for enhanced forest wildfire detection. 
