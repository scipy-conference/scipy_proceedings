...
Satellite image data usually requires significant storage and processing power, as each image contains a large amount of data across the chosen spectral bands and stores geographic information that needs to be preserved. Thus, the scale and speed of model training observed in the results below is constrained by these factors. 

For users with access to high-performance computing clusters or cloud-based GPU instances, processing times can be significantly reduced.
...
On the validation (val) set, the performance metrics exhibited positive trends as well. The validation accuracy began at 0.7843, peaked at 0.9359 around the eighth epoch, and remained high at 0.9333 by the final epoch. Validation, precision, and recall also showed improvements, with precision starting at 0.7688 and reaching up to 0.9606, while recall improved from 0.497 to 0.7757.
...
\begin{table}[H]
    ...
    \toprule
\textbf{Epoch} & \textbf{Accuracy} & \textbf{Precision} & \textbf{Recall} & \textbf{Val Accuracy} & \textbf{Val Precision} & \textbf{Val Recall} \\
    \midrule
