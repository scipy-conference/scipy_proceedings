...
.. [#] The easiest way to acquire the CHIANTI data is to download and unpack the tarball at `<http://www.chiantidatabase.org/chianti_download.html>`_. In order for ChiantiPy to find the database, it is necessary to point the :code:`XUVTOP` environment variable to the top of the CHIANTI directory tree. For example, if the database is downloaded to :code:`$HOME/chianti`, :code:`export XUVTOP=$HOME/chianti/dbase` should be placed in the Bash shell configuration file. 
