...
Here, we describe Ocetrac, an open-source Python framework designed for the detection, tracking, and quantification of irregular, evolving features in gridded climate data. Ocetrac combines morphological image processing (via scikit-image) with labelled array workflows (via xarray) to characterize irregular structures in gridded data. Its modular design supports: 
...
Originally developed for MHW analysis, Ocetrac can also be used for  other applications, from atmospheric rivers to hypoxic zones and bloom patches in the ocean. Its modular design allows users to customize detection thresholds, tracking logic, and metrics of tracked objects. This paper details its design, demonstrates several use cases, and outlines future directions. We demonstrate Ocetrac using extreme sea surface temperature (SST) anomalies to identify MHWs, but the approach is modular and can be adapted to other geophysical fields.
...
```python
from measures import cesm2_lens_utils
from measures import cesm_anomalies
...
from measures.intensity_measures import calculate_intensity_metrics, calculate_peak_timing
from measures.motion_measures import MotionMeasures
import ocetrac

## ---------- EXAMPLE WITH ONE ENSEMBLE MEMBER OF THE CESM2-LENS
var = 'SST'
comp = 'atm'
...
mean, trend, seas, features_notrend, var_notrend = cesm_anomalies.calculate_anomalies_trend_features(
    ds=var_ds_noland.sel(time=slice('1979-01','2014-12')), # only look at years 1979 to 2015
    threshold_perc=0.9) # 90th percentile threshold
...
Empirically, this sequence (closing followed by opening) optimizes feature integrity while minimizing fragmentation. The intermediate results of morphological operations are shown and described in greater detail in Scannell et al. (2024).
...
The choice of R represents a trade-off between feature completeness (higher R retains more connected areas) and spatial precision (lower R preserves finer-scale details) (Figure 2). Users should validate this parameter against known feature scales in their domain. Features, after undergoing morphological operations, are now referred to as objects.
...
```python
# PRE-PROCESSING
object_id = 226. # Choose a specific object ID to analyze
...
one_obj_anom = var_notrend.sel(time=true_time_steps.time) # only getting times where the event exists
only_one_obj = xr.where(one_obj == object_id, 1., 0)

masked_one_obj_anomalies = var_notrend*only_one_obj

# INPUT TO MEASURES SUBMODULE
masked_intensity = xr.where(masked_one_obj_anomalies > 0., masked_one_obj_anomalies, np.nan) # Intensity
one_obj['labels'] = only_one_obj # Binary event

## ---------- SHAPE
metrics = ShapeMeasures(lat_resolution=110.574, # km per deg lat
                       lon_resolution=111.320, # km per deg lon
...
for timestep in range(one_obj.time.shape[0]):
    coms = motion.calculate_coms(
        one_obj.labels,
        masked_intensity, timestep)
    com_coords.append(coms)

# Centers of mass displacements
com_path, com_displacements = motion.calculate_com_displacement(masked_intensity)
...
# Plotting
plot_displacement(centroid_path, one_obj.labels)
plot_displacement(com_path, masked_one_obj_anomalies)
...
We apply Ocetrac to the NOAA 1/4° Optimum Interpolation Sea Surface Temperature (OISST) dataset to demonstrate its functionality for real-world MHW analysis. This methodology for anomaly calculation and results is described in greater detail in Scannell et al. (2024). This study demonstrates Ocetrac successfully identifying known MHW events and their spatial structure, temporal evolution, and intensity distributions from the observational record. This shows how Ocetrac captures both large-scale and regional MHW events in an observational products. 
...
## Acknowledgments
The software development received funding from NSF, NCAR, the Leonardo DiCaprio Foundation Foundation, Microsoft, the Gordon and Betty Moore Foundation, and the University of Washington eScience Institute. We would also like to acknowledge the high-performance computing support from Casper provided by the NSF NCAR’s Computational and Information Systems Laboratory.