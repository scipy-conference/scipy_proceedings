...
This paper describes [Ocetrac](https://github.com/ocetrac/ocetrac/releases/tag/v0.1.6), an open-source Python package designed for detecting, tracking, and quantifying irregular, evolving features in gridded climate data. Ocetrac combines morphological image processing (using `scikit-image`, @van2024scikit) with labelled array workflows (using `xarray`, @xarray), to characterize irregular structures. The Measures submodule provides functionality to quantify spatial and temporal properties of the tracked events, and thus facilitates further statistical analysis, such as aggregation analysis of multiple events within large datasets. Its modular design supports: 
