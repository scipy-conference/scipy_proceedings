...
This paper describes [Ocetrac](https://ocetrac.readthedocs.io/en/latest/), an open-source Python package designed for detecting, tracking, and quantifying irregular, evolving features in gridded climate data. Ocetrac combines morphological image processing, using `scikit-image` [@van2014scikit], with labelled array workflows, using `xarray` [@xarray], to characterize irregular structures. The Measures submodule provides functionality to quantify spatial and temporal properties of the tracked events, and thus facilitates further statistical analysis, such as aggregation analysis of multiple events within large datasets. Its modular design supports: 
...
2. Quantification: Ocetrac computes shape, motion, and intensity measures
...
Objects outlined (purple outlines) detected using varying $R$ values ($R$ ranging from 1° to 7°). The background shows SST anomalies (°C) from February 2007 of an ensemble member of the CESM2-LENS dataset.
:::
