...
Our library `affinis` works as a systematic set of tools to consistently transform between _observational_ data on nodes (represented as binary feature activations), and the various forms of square matrices encounted when analyzing the way these features are _related_ to each other (or not).
...
The functions currently implemented and there primary use as documented are: 
...
: _methods:_ `coocur_prob`, `ochiai` [@Measuresecologicalassociation_Janson1981], `mutual_information`, `yule_y`, `yule_q`, `odds_ratio` [@MethodsMeasuringAssociation_Yule1912]
  \
  This class of association measures are derived from marginal counts alone, and care less about underlying structure between features and more about *how* observations are able to be counted/combined for statistical aggregation. The majority rely on observational units being *additive*, such that we can apply an inner product $X^TX$ on them.
...
Forest Pursuit, listed as a PGM, is  particular interest, since `affinis` contains the first reference implementation of this algorithm.
Forest Pursuit was recently proposed in []( https://doi.org/10.13016/o252-lfmi), and attempts to combine the local/additivity assumptions of the marginal methods, while _also_ directly recovering an underlying network of _conditionally dependent_ pairs of nodes (as with probabilistic graphical model). 
...
A comparison of several methods applied to an example feature matrix "dataset" can be seen in @fig:assoc-example.
...
Comparision of methods for edge estimation from bipartite projection. 
:::
...
Let's compare, since `affinis` makes applying pseudocounts to nearly all of the association measures _trivial_. 
...
$$
P = \frac{\textrm{successes}+\alpha}{\textrm{trials}+\alpha+\beta}
$$
...
We will note that `affinis.proximity` is not an exhaustive catalog of [existing kernels on graphs](https://doi.org/10.1016/j.ejc.2018.02.002), but does provide an interface to a few versatile functions for estimating and modifying kernels.
...
To enable broad cross-disciplanary participation and development toward improved network recovery from sparse/bipartite data, we propose a synthetically generated set of challenge problems, which we dub "MENDR": **M**easurement **E**rror in **N**etwork **D**iffusion \& **R**econstruction
...
Each json entry has attributes that directly correspond to instatiation parameters of sparse `COO` arrays from the [`pydata/sparse`](https://sparse.pydata.org/en/stable/) library.
...
In order to rapidly benchmark network recovery algorithms accross thousands of challenge datasets (as in MENDR), a different set of tradeoffs is needed when actually computing the performance---i.e. when _doing metrology_.
...
To report single scores once again, `Contingent` objects have a `.expected()` convenience method, which calculate the expected value for supported scores.
...
The user API is shown in @contingency-use, with performance benchmarks accross a wide range of dataset sizes shown in @fig:contingent-scale.
...
- sparsification
- more kernels
- Edge-vector representation generalization


### Dataset
- more+unique graph types
- jump-list recovery challenge
- I/O standardization (Binsparse, Hypergraph Interchange Format)
