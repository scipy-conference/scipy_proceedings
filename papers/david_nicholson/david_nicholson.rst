...
.. class:: abstract
    ...
    various algorithms. For example, there are methods using support vector machines (SVM), K-nearest 
    ...
    syllables from a given bird, and classifier accuracy was measured with large hand-labeled testing datasets for
...
The results in :ref:`fig2` showed that k-NN and SVM-RBF can yield higher average accuracy than linear SVM. However, the feature set for training the k-NN differed from the features for the SVM classifiers. A different feature set was used because of concerns that the 536-dimensional feature vector would yield poor results for k-NN as described above (and explored in-depth in [BEYER1999]_).
