...
abstract: |
  Current High Energy Physics measurements rely on onalysing the data by using underlying theoretical assumptions in model building that do not account for possible deviations from the Standard Model. This approach can introduce potential bias in parameter extraction and complicate the reinterpretation of measurements without fully reanalyzing the data.
  ...
  ReDist allows direct inference on the theoretical parameters underlying the kinematic distributions in any beyond the Standard Model scenario by providing weighted binned histograms of the kinematic distributions to any alternative model of interest.
  ...
  We present a ReDist-HAMMER interface that has been developed to address direct Beyons the Standard Model measurements by reinterpreting existing datasets collected by various High Energy Physics collaborations, using the increasingly popular theoretical backend HAMMER. This enables model-agnostic interpretation of data—a crucial step for advancing precision flavor physics.
...
## Introduction
  ...
  These inherent dependencies make reinterpretation of datasets collected by the the LHCb [@Alves:1129809] and Belle [@brodzicka2012physicsachievementsbelleexperiment] collaborations, leading the field of flavour physics, challenging.
...
:::
  ...
  The ReDist-HAMMER [@lorenzennio_redist] is the first full Pythonic interface between the HAMMER package and a fitting environment, by integrating it with the pyhf framework [@pyhf] [@pyhf_joss].
  This allows HAMMER-processed samples to be used directly within pyhf’s binned-likelihood models [@Cranmer:2012sba], via the ReDist custom modifier of the likelihood, encoding the BSM and FF dependence of the shapes as defined by the HAMMER theoretical backend.
...
## The PyHF and the ReDist package
  ...
  ```
  where the index $i$ represents the two types of constraints, and $a$ represents the auxiliary data defining the constraints.
  ...
  The problem with these definitions of the rate, in terms of the modifiers, is how complex it would be to represent very convoluted dependencies of the rates if we want the degrees of freedom $\eta$ and $\chi$ to be the NP coefficients defined in the HAMMER package, or the FF parameters.

  The ReDist package [@G_rtner_2024] addresses this problem with a reinterpretation method where the distributions of the observables are updated on the basis of the new kinematic assumptions.
  The method has been implemented in the *pyhf* fitting interface through the definition of the custom modifier of the likelihood, a modifier that varies the rates calling external, and custom, functions which *return* is computed by the theoretical backend.
...
## Outlook
  ...
  Allowing a correct reinterpretation of data also opens the possibility of combining different datasets in a single coherent fitting environment.
