...
In addition to the the change in the 100 dimensioned vector between each time bucket and it's next, the initial and next 10 dimensional embeddings were included from each, which were generated using the same dynamic embedding model.
...
The other experimental data set was constructed similarly, but rather than subtracting the two vectors and including lower dimensionsed vectors, the initial and final 100 dimensioned vectors were listed as features.
This allowed machine learning algorithms to have acess to the full positional information of each vector along side the shift between the two.
This yeilded each word having 17 observations and 201 features: {vec_init0 … vec_init99, vec_fin0 … vec_fin99, label}, and this data set will be referred to as "data_201".
...
All these features were dropped prior to use within the models, as the magnitude feature was held colinearity with the other positional features, and the case and day data will not be availiable in predicting modern outbreaks. 
Using these data, two machine learning alogrithms were applied: unsupervised k-means clustering, and a supervised neural network.

K-means Clustering
==================

To try to examine any similarities within shifts, k-means clustering was performed on the data sets.
Initial attemps at k-means with the d = 100 dimensioned embeddings yeilded extremely large intertial values, and poor results. 
...
After the vocabulary was split, the corresponding observations were split accordingly.
...
The end goal is to create a pipeline that can perform semantic shift analysis at set intervals of time within these counties, and classify these shifts on the neural network trained on the Scott County data. 
