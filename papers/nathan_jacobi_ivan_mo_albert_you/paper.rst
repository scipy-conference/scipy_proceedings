...
The certainty threshold for the unmixed split in both sets was .01, and .1 for the mixed split both sets.
...
.. table:: Results of the neural network run on the data_201 set. The epochs column shows the number of training epochs on the models, as well as if the words were mixed between the training and testing data, denoted by "Mix". :label:`nn201`
   :class: w
...
Additionally, accuracy on data_201 begins to increase between 150 and 200 epoch models on the no mixed split, so even higher epoch models might improve results further on the no mixed split.
