...
We present `alphagenome-pt` ([PyPI](https://pypi.org/project/alphagenome-pt/), [GitHub](https://github.com/RylieWeaver/AlphaGenome_PyTorch)) an open-source PyTorch implementation of Google DeepMind’s AlphaGenome model architecture for training and fine-tuning. DNA language models have become an active area of genomics research to learn the latent structure of DNA from large collections of genomic sequences [@AlphaGenome-Nature-2026; @Enformer-Nature-2021; @DeepSEA-Nature-2015; @Basenji-Nature-2018; @Borzoi-Nature-2025; @SpliceAI-Nature-2019; @Pangolin-PubMed-2022; @EVO2-Nature-2026; @EVO-nguyen-2024; @PlantCAD2-2025; @NucleotideTransformer-Nature-2025; @NucleotideTransformer-biorxiv-2023; @DNABERT-2-arxiv-2024; @DNABert-Bioinformatics-2021; @HyenaDNA-NeurIPS-2023]. Within this broader area, one important direction is to predict function directly from DNA sequence, which is a central challenge in genomics. AlphaGenome advances this direction by predicting a wide range of functional genomic outputs, including gene expression, chromatin accessibility, splicing, and chromatin contact maps, directly from DNA sequence for human and mouse genomes [@AlphaGenome-Nature-2026]. These predictions are valuable because they provide fast computational estimates for quantities that would otherwise require relatively expensive and time-consuming experiments. Moreover, DNA sequence inputs can be precisely specified and systematically modified in silico, enabling controlled perturbations that are difficult or impossible to perform experimentally. A sufficiently accurate and general model could accelerate progress in biology across major sectors such as healthcare, pharmaceuticals, agriculture, energy, biotechnology.

However, Google DeepMind’s released AlphaGenome models are trained on human and mouse genomes, limiting their direct application to other species. Practitioners who want to apply AlphaGenome to other species may therefore need to train or finetune models with the same core architecture on species-specific data. At the same time, the released implementation is written in JAX [@JAX-GitHub-2018], while many deep learning workflows in the scientific Python community are built in PyTorch [@PyTorch-NeurIPS-2019]. `alphagenome-pt` addresses this gap by allowing researchers to instantiate, modify, train, and checkpoint AlphaGenome models in PyTorch without reimplementing the model themselves. By lowering the software barrier to training and fine-tuning AlphaGenome, `alphagenome-pt` aims to make the model easier to adapt to new biological settings.
...
The most closely related software is the official AlphaGenome release from Google DeepMind ([Link1](https://github.com/google-deepmind/alphagenome_research), [Link2](https://github.com/google-deepmind/alphagenome)). That implementation remains the reference implementation for the model and released checkpoints, but it is written in JAX, whereas `alphagenome-pt` is motivated by the complementary goal of making the model trainable in PyTorch. There are also other unofficial PyTorch implementations of AlphaGenome. The [`genomicsxai/alphagenome-pytorch`](https://github.com/genomicsxai/alphagenome-pytorch) package is a strong and polished PyTorch port, with pretrained checkpoint loading, named outputs, example notebooks, fine-tuning utilities, and reported numerical parity with the original JAX model. In contrast, `alphagenome-pt` is focused on flexible model training. In particular, `alphagenome-pt` includes a masked-language-modeling pretraining head, exposes more architecture hyperparameters for modification, and implements RMS batch normalization using current-batch statistics during training. This differs from the `genomicsxai` implementation, where RMS batch normalization uses stored running variance in the training forward passes. As a result, `alphagenome-pt` allows training gradients to flow through the normalization statistics induced by the current batch, which may be important when training AlphaGenome models from scratch or substantially changing their training distribution. A previous PyTorch implementation by Phil Wang, Miquel Anglada-Girotto, and Xinming Tu was also created but has since been taken down from GitHub.
