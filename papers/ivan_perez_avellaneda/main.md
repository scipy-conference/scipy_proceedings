...
```
To make notation lighter, we skip the limits of the integrals and its differentials. Repeating the integration process 
...
```{code} python
:label: MBB_py
:caption: The reachable set of the susceptible population is obtained by computing its optimal values generated by the inputs
 $\beta$ and $\gamma$ entering the system.
import numpy as np
import sympy as sp
