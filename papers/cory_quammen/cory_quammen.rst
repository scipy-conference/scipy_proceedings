...
communicate among instances of themselves in a distributed computing
environment, ParaView wraps designated VTK classes in proxy classes
that are able to communicate with each other among distributed
processes.
...
process. The ``paraview.simple`` layer simplifies this process with a
set of high-level functions that take care of most of the tedium. The
same example above expressed in ``paraview.simple`` functions is
reduced to
...
VTK and ParaView currently support Python 2. Support for Python 3 is
targeted for sometime in 2016.
