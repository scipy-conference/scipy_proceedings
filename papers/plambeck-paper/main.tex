...
This paper extends an earlier OSF preprint of the same study \cite{PlambeckPuleriGonzalez2025}, incorporating revisions from the SciPy review process: an explicit model-implied-vs-realised uplift distinction in the abstract and Results, de-jargoned terminology, and first-use definitions for TRx, NBRx, and detailing.

Section~\ref{sec:data} describes the data; Section~\ref{sec:methods} covers the modelling pipeline and territorial constraints; Section~\ref{sec:mip_overview} formalises the optimisation; Section~\ref{sec:stats_eval} defines the evaluation methodology; Section~\ref{sec:results} reports results; and we close with limitations and future work.
...
Our hypothesis is straightforward: embedding spatial-peer context and hard operational constraints in a joint learning--optimisation pipeline will yield materially higher incremental TRx and NBRx than current practice using the same inventory.
...
\begin{table}[ht]
  ...
  \begin{tabular}{@{}p{0.45\linewidth} p{0.45\linewidth}@{}}
    ...
    FTR\_VAR\_002 & Target outcome variable A (e.g., predicted behaviour over 13 weeks) \\
    ...
    \bottomrule
  \end{tabular}
  \caption{Model Feature Categories and Abstracted Variables. \textit{(Actual feature names have been anonymised for proprietary reasons.)}}
  \label{tab:variables}
\end{table}
...
\begin{figure}[ht]
  ...
  \caption{Pipeline schematic for the proposed framework. Data engineering constructs rolling-window HCP-level predictors and look-forward outcomes; machine learning models estimate band-specific response surfaces; scenario generation creates feasible HCP-level sample choices subject to historical and operational bounds; the mixed-integer programme selects one scenario per HCP under HCP-level and territory-level constraints; a top-off heuristic spends residual inventory; and the final field pack reports HCP ID, territory ID, prior allocation, optimised allocation, and projected response.}
  \label{fig:pipeline_schematic}
\end{figure}

\subsection{Training Data Preparation}

We extract the raw observational panels for each HCP from Pfizer's North American commercial data environment and perform upstream filtering, joins, and aggregation before modelling. The resulting analysis tables are then transferred into Python for feature engineering and estimation \cite{RahmanMueller2021,Chamberlain2022}.

Two binary indicators, \texttt{IS\_TRX\_ZERO} and \texttt{IS\_SAMPLES\_ZERO}, are created in-database. For every healthcare provider \(i\) observed over \(T_i\) weeks we compute panel sparsity as
...
= \frac{1}{3T_i}
  ...
  \Bigr],
\end{equation}
and retain only those panels whose average sparsity \(\bar r_i\) does not exceed \(r^* = 0.30\). Panels shorter than the minimum length \(T_{\min}=100\) weeks are dropped, and no more than \(R_{\max}=1.2\) million unique provider-week records are processed in a single run. Randomised batching (500 IDs per chunk) prevents memory spikes while the global row limit guarantees that downstream feature engineering remains in-core.
...
These form the territorial constraints for our mixed-integer programme that will be formalised in a later section:
...
Predicted growth scores rise markedly across the three HCP value bands defined earlier. Bands 0 and 1 exhibit overlapping, low central tendencies and modest dispersion, reflecting noisier response among lower-value prescribers. Band 2 shows a clear rightward shift, with a substantially higher median and upper quartile, indicating materially greater predicted TRx uplift for the top 40\% of physicians. This confirms that the highest-value band contains the most responsive HCPs and underpins our decision to focus optimisation where marginal returns are greatest.
...
\noindent\textit{Note:} the figure legend numbers the bands ``1--3'' rather than ``0--2'' (Legend Band $n$ corresponds to our Band $n-1$); we will correct this in a future revision.
...
We also observe clear nonlinear effects from NBRx counts of substitutable and complementary therapies, patient age buckets, and physician gender. Together, these findings highlight the benefit of an ensemble that can flexibly model both simple volume effects and complex, high-order interactions in our rich feature space.

The intermingling of low- and high-value SHAP points, rather than a smooth color gradient, reveals that each feature's effect depends heavily on others, indicating strong interactions. This motivates more granular modelling: for example, grouping HCPs into latent clusters (via unsupervised embeddings or mixture models) and then fitting specialised submodels or introducing targeted feature crosses within each cluster to better capture those interactions.
...
Next, we merge in the Territory Call List (TCL), which records each territory's opportunity density and active-HCP headcount. From these we derive per-territory pill-budget floors \(\mathrm{TERR\_MIN}_t\) and ceilings \(\mathrm{TERR\_MAX}_t\). The CVXPY modelling layer then solves the following 0--1 mixed-integer programme using CBC:
\[
\begin{aligned}
...
\begin{table}[ht]
  ...
  \begin{tabular}{@{}lr@{}}
    ...
    Rows optimised                                    & 95,278         \\
    Total recommended pills dropped (blue)            & 452,001        \\
    Total maximum capacity (orange)                   & 451,992        \\
    Utilisation ratio (blue/orange)                   & 1.000          \\
    Capacity efficiency (pre--top-off)                & 97.9\%         \\
    Top-off threshold                                 & 95\%           \\
    Threshold reached                                 & Yes            \\
    \bottomrule
  \end{tabular}
  \caption{Optimiser Territory Efficiency Summary}
  \label{tab:territory_efficiency}
\end{table}

\section{Lift Assessment}
We measure uplift by comparing each HCP's optimised allocation against their historical ``status-quo'' level. Let
\[
  m_i = \mathrm{median}\bigl\{s_{i,t}\bigr\}
\]
be the median sample-drop count for HCP \(i\) over the six-month history, and let \(x_i^*\) be the optimiser's recommendation. We then compute
\[
  \hat y_i(m_i)
  \quad\text{and}\quad
  \hat y_i(x_i^*)
...
Table~\ref{tab:uplift_diagnostics} summarises the model-implied expected uplift and the results of the Kruskal--Wallis test comparing the empirical versus optimised fitted TRx distributions. Table~\ref{tab:descriptive_stats} reports side-by-side descriptive statistics for those two distributions.

Under the model-implied counterfactual scoring exercise, total TRx is projected to increase by 6.4\%, highlighting the practical significance of the optimisation framework. This 6.4\% should be read as a model-implied upper reference rather than a forecast of realised field impact: after deployment we expect the realised gain to be lower, on the order of 1 to 3\%, because operational frictions, representative discretion, sample availability, and market-response uncertainty attenuate optimised recommendations. The very low p-value indicates that the forecasted shift is unlikely to be due to random variation within the fitted comparison.
...
The empirical TRx distribution has a mean of 0.2476 and a large standard deviation of 0.9625, reflecting high variability and many low-activity physicians. In contrast, the optimised TRx distribution shifts right to a mean of 0.2634 with similar spread, consistent with the 6.4\% expected uplift reported above, indicating the optimisation effectively reallocates samples towards higher expected returns without unduly increasing variance.
...
\begin{table}[ht]
  ...
  \begin{tabular}{@{}l r r@{}}
    \toprule
    Statistic      & Empirical TRx & Optimised TRx \\
    \midrule
    Count          & 94,912        & 94,912        \\
    Mean           & 0.2476        & 0.2634        \\
...
Figure~\ref{fig:diff_means} displays the difference in means between the optimised and empirical fitted responses for each HCP, highlighting the shift of the entire distribution toward higher expected TRx under the optimised allocation.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.8\linewidth]{figures/figure22.png}
  \caption{Difference of Means -- Fitted vs. Optimised-Fitted Response}
...
\textbf{Projected economic impact:} Counterfactual scoring indicates a model-implied 6.4\% lift in total TRx relative to status-quo allocation (\(p<10^{-30}\)), suggesting meaningful upside at constant inventory rather than realised causal revenue lift.

While encouraging, these gains remain model-dependent, and future work will pursue the causal validation and rolling-horizon extensions discussed above. The present results nonetheless show that a tightly coupled learning-and-optimisation stack can respect complex operational constraints and support materially improved allocation plans across therapeutic areas and markets.

\section*{Appendix}

\subsection*{Ethics, Data Governance, and Reproducibility Note}
This study uses de-identified, HCP-level commercial and operational data for internal planning analysis and does not involve patient-level intervention or clinical randomisation in its present form. Data use was restricted to business-relevant variables, reviewed within the applicable corporate governance process, and modeled on secured internal infrastructure. Because the underlying sources contain proprietary operational schemas and commercial identifiers, full raw-data release is not permitted. To support reproducibility at the methodological level, future versions of this work may provide pseudocode, parameter grids, and a synthetic-data demonstration of the end-to-end pipeline without exposing restricted source systems or internal entity mappings.
