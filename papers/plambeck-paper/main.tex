...
This paper extends an earlier OSF preprint version of this work \cite{PlambeckPuleriGonzalez2025}. Relative to that preprint, the present version incorporates revisions made during the SciPy review process: it distinguishes explicitly between the model-implied projected uplift (\(\sim\)6\%) and the more conservative realised field-impact expectation (1--3\%) in both the abstract and Results rather than only in the Discussion, de-jargons terminology for a general computational audience, and defines TRx, NBRx, and detailing on first use.

The remainder of the paper is organised as follows. Section~\ref{sec:data} describes the HCP--week panel, data sources, and engineered feature categories. Section~\ref{sec:methods} details the modelling pipeline, including sparsity filtering, the log-plus-one response transformation, spatial-lag feature construction, and the territorial constraints that bound the optimisation. Section~\ref{sec:mip_overview} formalises the mixed-integer programme that selects a sample-drop scenario per HCP, along with the top-off heuristic used to spend residual territory capacity. Section~\ref{sec:stats_eval} defines the statistical evaluation methodology, including the expected-uplift metric and the Kruskal--Wallis significance test. Section~\ref{sec:results} reports model fit and partial-dependence results, followed by growth-score distribution and response-curve analyses, feature importance, and the optimisation and lift-assessment results. We close with a discussion of limitations and directions for future work.
...
\noindent\textit{Note:} in the current figure the legend labels the bands as ``1--3'' rather than ``0--2.'' Specifically, Legend Band 1 corresponds to our Band 0, Legend Band 2 to Band 1, and Legend Band 3 to Band 2. We apologise for any confusion and will correct the legend in future versions.
