...
One model which provides a more fertile ground for sociological analysis is the structural cohesion model [moody2003]_. This model is grounded on two common conceptualizations of group cohesion in the literature. A social group is considered cohesive to the extent that: a) it is resistant to being pulled apart by the removal of some of its members; and b) pairs of its members have multiple direct or indirect connections that pull it together. Building on the concept of node connectivity [#]_ from graph theory, the structural cohesion of a group is defined in this model as the *minimal number of actors who need to be removed from the group to disconnect it*. Despite its solid and elegant mathematical foundation, the structural cohesion model has not been widely used in empirical analysis because it is not possible to perform the required computations for networks with more than a few thousands nodes and edges in a reasonable time frame.
...
.. [#] You can download the networks used in this section in graphml format. Nodes have an attribute named *bipartite*, with values 0 and 1, which indicates the node set to which each node belongs. Note that this is the convention used in NetworkX bipartite package (see https://networkx.github.io/documentation/latest/reference/algorithms.bipartite.html):

    * Debian Lenny: https://github.com/jtorrents/structural_cohesion/blob/master/data/debian_lenny_5.0.graphml

    * Nuclear Theory: https://github.com/jtorrents/structural_cohesion/blob/master/data/nucl_th_2006_2011.graphml

    * High Energy Theory https://github.com/jtorrents/structural_cohesion/blob/master/data/hep_th_2006_2011.graphml
