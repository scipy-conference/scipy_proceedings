...
A failed guard flags a change for closer review without automatically rejecting it. The relevance judge illustrates this: enabling it lowered officer completion from 95% to 92% by vetoing wrong-article completions, which trips the target guard. The change shipped because the judge had earned its authority in the offline audits. In the civilian audit, all 17 vetoes among 123 reviewed records were genuine wrong-article matches with none false, and the officer audit reached the same verdict on its smaller veto set, and the hard guards held (zero hallucinations, no cohort-correctness drop). Analyzing the ten held-out vetoes (seven civilian, three officer) confirmed each one genuine. Three of the discarded extractions took values from coverage of officers killed in unrelated, well-known cases. The gate's value is that it made the completion-for-faithfulness trade explicit and measurable.
