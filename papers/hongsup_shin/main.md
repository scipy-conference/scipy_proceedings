...
LangGraph models a pipeline as a graph whose nodes are plain functions and whose state is a single typed object that each node reads and returns. We use it narrowly on purpose. The state schema is the `EnrichmentState` `pydantic` model, so every field is validated and the whole run is one examinable object, and there is exactly one conditional edge, out of the Coordinator, whose target is chosen by a deterministic `match` on the stage that just ran (simplified from `src/agents/coordinate_node.py`):
...
This sits at the opposite end of the autonomy spectrum from the agent abstractions the same ecosystem also offers: where LangChain's agent executors, CrewAI's crews, or AutoGen let a model choose tools and coordinate other agents, LangGraph exposes the graph and its typed state directly [@langgraph; @langchain], giving us the deterministic and transparent control we wanted. The one routing outcome attributable to a model judgment is the relevance judge's veto, which escalates a wrong-article record. Its sibling escalations, on empty or conflicting extractions, are model-agnostic completeness guards that fire the same way for a regex extractor. The veto has no rule-based equivalent. The Coordinator reads the veto like any other state field: the model supplies the judgment and deterministic code executes the move. Runtime dependencies (the LLM client, the `Settings`) are injected through the framework's `RunnableConfig`, so in production a node reads a Claude [@anthropic_claude] client and in unit tests a `MagicMock`, which makes the agentic judges testable without a live model or network.
...
A record *completes* when it reaches the Complete node with at least one field extracted; otherwise it escalates to human review. On the held-out set (100 records per dataset; {ref}`tbl:holdout`, {ref}`fig:overview`), the pipeline completes 70% of civilian records (95% Wilson interval 60–78%) and 92% of officer records (85–96%); officers complete far more often because officer-involved shootings draw denser news coverage, so retrieval rarely fails. Among extracted values, aggregate exact-match precision is 77% for civilians and 71% for officers, rising to 89% and 86% under fuzzy match. Of the 30 civilian escalations ({ref}`tbl:outcomes`), 22 are retrieval gaps (no articles found), 7 are relevance-judge vetoes, and 1 is insufficient sources; the officer escalations split 4/3/1 the same way. No record escalates on a conflict alone, since partial completion commits the agreed fields and routes only the contested one to review.
...
The system is built for a sensitive domain, and several commitments follow from that. It keeps a human in the loop, never overwrites original databases, prefers faithfulness over coverage, and refuses to infer race from proxies. It also makes every suggestion auditable: each extracted value is written with its confidence label and the source URLs (and, in the escalation reports, the verbatim conflicting values) a reviewer needs to check it against the original coverage.
