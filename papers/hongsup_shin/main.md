...
Cost discipline raises an obvious question: if Haiku is the cheaper model, why not run the whole pipeline on it? We treated this as another *earn-it* decision and let the gate answer. The lever is real but bounded — Haiku is priced at a third of Sonnet per token, on both input and output, so the LLM share of the ~\$400 full-archive estimate would fall by about two-thirds, not the order of magnitude the framing "just use the cheap model" implies. Following the project's offline-first discipline, we sized that saving on saved runs before spending a dollar, then bought the cheapest live signal first: the same twenty-incident adversarial probe, re-run with extraction and both binary judges moved to Haiku. Sonnet fabricated nothing on that probe; the all-Haiku variant committed one — asserting the trap's planted victim name, "Michael Brown," with high confidence on the common-name-confusion scenario, exactly the failure the probe was built to catch. Under the gate that single fabrication is a hard, never-overridden veto, so the variant was rejected without even running the holdout. This is the negative companion to the gate's earlier positive demonstration: the same guard that let us ship a completion-lowering change also refused a tempting cost cut, because in this domain a confidently wrong field is worse than the cost it would save. We therefore keep the major runs on Sonnet, reserving Haiku for the conflict annotator's advisory notes — where, because it never commits a value, the cheaper model carries no faithfulness risk.
