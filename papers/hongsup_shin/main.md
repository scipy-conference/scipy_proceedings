...
abstract: |
  The Texas Justice Initiative (TJI), a nonprofit, publishes some of the only granular databases of officer-involved shootings in Texas, but many records are missing fields that matter for accountability: the weapon involved, the subject's race and age, the outcome. Volunteers recover these by hand from news archives, fifteen to thirty minutes per record. We automate that workflow with a pipeline built on LangGraph and the Anthropic API that searches the web, extracts fields, and escalates hard cases to a human. The central finding is an honest one: the recovery itself came from a *deterministic* prompt fix, with no agent required. What we did ship is a thin *faithfulness* layer of three bounded large language model (LLM) judges with graduated authority (block, null, advise), and each had to clear an offline, multi-objective evaluation gate before it was allowed to run, because in this domain a wrong value is worse than a missing one. On held-out samples the pipeline completes 92% of officer records and 70% of civilian records, and across twenty fabricated incidents it invented zero facts. The contribution is a discipline rather than an algorithm (an "earn-it" protocol, a reward-hacking-resistant evaluation gate, and authority calibrated to stakes) for deciding *where* agentic judgment belongs in high-stakes data work: in the judgment layer, above extraction.
---
