...
described the shortcomings of widely used astronomical standard format
...
We will give an example more advanced use case that illustrates some
...
but a much larger field of view than HST, will be launched in 2026 or thereabouts. It will use ADSF as its primary data format.
...
Moving the URL authority from ``stsci.edu`` to ``asdf-format.org``
..................................................................
...
A more straightforward way of associating tags with schemas, allowing 
...
these associations were derived from class attributes but distributed
throughout the code to deal with the conversion process).
...
We now recommend the tag URIs begin with ``asdf://``
...
proving more intuitive names for the methods and attributes, and makes
...
optional magic method that the info mechanism can take advantage
of.

Plugin API for block compressors, extensions and schemas (2.8.0)
................................................................
...
Despite the fact that the models are in some sense executable, they are perfectly safe so long at the library they are implemented in is safe (e.g., it doesn't implement an "execute any OS command" model). Furthermore, the representation in ASDF does not explicitly use Python code. In principle it could be written or read in any computer language.
...
.. figure:: figure1.png
   ...
   A plot of the compound mode defined in the first segment of code.
...
There are a number of components that are involved. Converters involve
the code that handles converting Python objects to and from the ASDF
content. These are classes that inherit from the basic ``Converter`` class
...
the schema (as well as proving extra information for the ASDF content
...
and engineering community, and such items increase the priority
...
We expect to layer the support in out Python library on 
``zarr`` [https://zarr.dev/], with
two different representations, one where all data is contained within
the ADSF file in separate blocks, and one were the blocks are saved in
...
uses is important. This typically means making the format work with 
S3 efficiently.
...
aren't quite dead yet https://www.youtube.com/watch?v=QcbR1J_4ICg). So making ASDF data at least readable to
