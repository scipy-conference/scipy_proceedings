...
Every number, table, and figure below is regenerated by the two notebooks in
`examples/paper_notebooks/` of the `shap-monitor` repository [@shapmonitor], one
per study. Both fetch the dataset directly, so no manual data preparation is
required.
