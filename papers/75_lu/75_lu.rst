...
So why bring all this up? Clearly, values encoding to negative values along a
basis vector would prove problematic. But it offers the prospect of further
reducing dimensionality by encoding to not just the basis a vector but also its
antipodal vector. To further our graphical example, in figure
:ref:`fig:sphere`, we use :math:`(1,0)`, :math:`(0,1)` and their antipodal
vectors :math:`(-1,0)` and :math:`(0,-1)` to encode values and apply a softmax
using those vectors.
