...
The anomaly-type classification task was much harder than binary detection. The best model, Random Forest trained on the full SciPy feature set, achieved macro-F1 = 0.4578 across five anomaly mechanisms. Each mechanism contributes 3,360 traces to the full anomalous dataset and exactly 840 traces to the stratified test set (4,200 test traces total). The Random Forest result is well above both the raw-signal Extra Trees baseline (macro-F1 = 0.2051) and a most-frequent-class dummy baseline (macro-F1 = 0.0667). On these balanced classes, the dummy has accuracy and balanced accuracy of 0.20; its macro-F1 is lower because it assigns every trace to only one class.
