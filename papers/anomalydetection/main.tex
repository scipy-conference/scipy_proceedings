...
The core implementation calls
\texttt{signal.savgol\_filter(x, window\_length=51, polyorder=3,
mode="interp")} and computes the residual as \texttt{x - smooth}. Its energy,
absolute mean, and maximum magnitude are then computed with NumPy reductions.
For spectral features, the implementation calls
\texttt{signal.welch(x, fs=fs, nperseg=128)} and integrates each selected
frequency band with \texttt{simpson}. The same power spectral density supplies
spectral entropy, dominant frequency, and high-frequency ratios.
