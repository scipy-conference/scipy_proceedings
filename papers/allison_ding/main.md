...
abstract: |
  Synthetic data generation (SDG) plays an increasingly important role in modern machine learning workflows. It addresses the dual challenges of data scarcity and privacy constraints, which often prevent the use of real-world datasets in sensitive domains such as healthcare, law, and finance. Although progress has been made in the structured data domain, especially with tools like SDV and CTGAN, the generation of high-quality synthetic text remains a challenging and underdeveloped area. Many existing SDG pipelines lack mechanisms for semantic control, offer limited quality assurance, and are not designed with computational efficiency in mind.
...
The initial stage involves loading raw data, typically stored in JSONL format. The dataset we use as an example contains question-answer pairs and optional metadata such as titles, tags, or confidence scores. The data then undergoes a multi-step cleaning process to remove HTML tags, correct malformed Unicode characters, and apply filters based on word count and quality scores. For example, questions and answers are retained only if their lengths fall within acceptable ranges and their associated scores exceed a defined threshold. This step is crucial for establishing a high-quality seed set and preventing malformed data from degrading the performance of downstream generating and scoring models. NeMo Curator supports several heuristic filters that you can choose to clean your data.  
...
def SDG_Rewards_Pipeline(dataset, round_num, sample_size_perc = 0.001, rewards_threshold = 0, n_variants = 1):
...
Users can enable this acceleration by simply loading the RAPIDS extensions at the beginning of their notebooks, without needing to modify their existing pandas or scikit-learn code. This transparent acceleration allows the pipeline to interactively process tens of thousands or even millions of records, making it practical for rapid prototyping as well as scalable production workflows.

```python
# Activate GPU Acceleration of cuML and cuDF
...
synthetic candidates in accumulation, the dataset size only increased slightly, reflecting strict enforcement of semantic quality constraints. 

### Visual Outputs

Reward score histograms further supported the filtering logic by revealing clustering around acceptable normalized reward thresholds and removal of semantically duplicated phrasing. These visualizations confirm that the semantic filter is not only size-preserving but also meaningfully content-selective.

The plots below compare the normalized rewards score distribution of the raw dataset (19,474 records) with the normalized rewards score distribution of the curated dataset (77 records). The raw dataset shows a broad and dense distribution across scores, reflecting high variability in quality. In contrast, the curated dataset demonstrates a much more concentrated score distribution, highlighting a focus on higher coherence, factual correctness, and overall content quality. The higher normalized reward scores in the curated dataset illustrate the effectiveness of the semantic filtering and reward-based selection in retaining only the most valuable and accurate samples.

:::{figure} Figure2.png
:label: fig:raw
Normalized Rewards Score Distribution of Raw Data
:::

:::{figure} Figure3.png
:label: fig:curated
Normalized Rewards Score Distribution of Curated Data
:::
