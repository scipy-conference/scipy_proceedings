...
Introduction
------------
Studying the dynamics of biological cells is key to understanding key biological questions. Such questions can be studying how cell decide their fate in developing animal embryos and how changing the experimental conditions can change their fate. Cells are often tagged with fluroscent proteins which express over time and such expression can be recorded as intensity changes of the cell which is recorded as a time lapse sequence. For some such and many other biological questions microscopy is a key technique to obtain that data. Once the data has been collected the analysis requires use of image analysis techniques which have received a major boost in bio-image processing since the advent of deep learning :cite:`Rasse2020`. Image analysis almost always requires segmentation of cells/tissues, tracking developement of tissue or movements of cells over time. Imaging conditions can be highly variable and have different sources of noise which degrades the quality of the image, this makes the task of segmentation tedious and of recent we can use deep learning based restoration techniques for denoising such images to make the downstream analysis easy :cite: `Florian2019`. Such restoration techniques enable easy segmentation of cells using conventional computer vision based tools. The restoration training often requires creating image pairs of high and low noise and it could be not possible for certain organisms or microscopy techniques (Claudia your example maybe?). However if the restoration is neccesary for just having good segmentations  a direct approach to segment the cells/tissues can be made inexpensively by annotating images. Many tools for annotation are available such as Qupath :cite:`Bankhead:2017wr`, Labkit, paintera, Napari etc. Using these tools and some freely available training datasets a direct segmentation training in 2D and 3D can be come. Reliable segmentation is neccessary for any tracking software to track cell/tissue motion. Cell tracking is also challenging in biology as cells have an erratic motion and can move in and out of frames creating gaps in tracks. Using simply the cetroid information to track the cells is not very accurate in such cases and requires custumizable cost function matrix to link the cells int he first instance and then link the segments of the connected cells in the second instance. The first instance is always needed to have a high accuracy of cells being linked based on their size/shape and intensity attributes and the second instance of linking segments is needed to account for dividing and splitting trajectories.
