...
At a conceptual level the methods supported in Emukit -- such as Bayesian optimisation, experimental design and Bayesian quadrature -- are all cyclic\maren{`Sequential' or `iterative' may be the better word as the state from one iteration to the next changes. `Cyclic' implies that the states are identical.} decision making processes that follow a similar pattern. Algorithmically they can be thought of as instances of a common abstract loop, which we now describe (also see Algorithm~\ref{alg:emukit-loop}).
