...
\maren{general comments:
  \begin{itemize}
    \item decide on UK or US spelling.
    \item add citations where missing.
    \item unify macros and spelling, e.g, decision making or decision-making; Emukit or EmuKit?
    \item Section referencing does not seem to work.
  \end{itemize}
}
\andrei{thoughts:
\begin{itemize}
    \item we mostly use UK in the docs. let's roll with that
    \item i can help to some extent with this
    \item "decision making", "Emukit". are there more?
    \item that's weird! shall we raise this with the proceedings repo?
  \end{itemize}
}
...
Data selection is a major challenge in supervised machine learning (ML). Quite often when data is abundant, it is collected prior to the training process and provided statically, meaning that the machine learning model has not influence on the data collection process. However, if data is expensive and scarce the performance of a model trained on a static dataset can be suboptimal or even poor \maren{cite}. In those cases, it is beneficial to carefully select the dataset such that, for example, it is maximally informative under the ML model to achieve the task at hand. This branch of ML is generally called \emph{active learning} \maren{cite} and has attracted attention in various sub-fields such as \emph{experimental design} (the task of predicting an unknown function value from its input), \emph{global optimization} (guessing the global minimizer of a function) and \emph{integration} (guessing the integral of a function from function evaluations).
...
\item Emukit can use existing, potentially specialized, custom models provided by the user and wrap them into a provided interface. As such Emukit is \emph{model backend agnostic}.
 ...
 \item Emukit provides basic functionality for \emph{multi-fidelity modeling}, \emph{sensitivity analysis} and \emph{benchmarking} \maren{Do we want to use the word benchmarking? As far as I remember that term may be ambiguous.} Further, Emukit contains a limited number of model wrappers to illustrate their usage and some example applications.
\end{itemize}
...
This section gives an overview over the machine learning methods provided by Emukit.
...
Finally, Emukit provides basic support of \emph{multi-fidelity models} \maren{cite} which can combine query results of the black-box function of different quality (from low fidelity to high fidelity). This yields multi-fidelity BO, BQ and ED methods that can be made active again with Emukit's decision loop. \maren{add example}

\maren{A word here on simulation and emulation and wording, depending on how it is handled in the introduction section.}
...
\begin{algorithm}[H]
  \caption{Decision making loop in Emukit (taken from the original NeurIPS workshop paper \cite{paleyes2019emulation}).}
...
\andrei{Maren, would you like to write a paragraph about your work on BQ with Alex G.? Or the one with Masha? Or both?}\maren{Yes, I can do that. Although the code of both papers unfortunately is not available online currently.}
...
\maren{The shared core package may need to be a section on its own as its quite a unique feature of Emukit, perhaps including the diagram/graph that is also in the rtd. I added a placeholder subsection above. What do you think?} \andrei{I tried it, and bq-bo-bq does work, see snippets. I've added your points, slightly rephrased, there, and briefly repeated them here.}
