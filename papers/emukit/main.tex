...
\item Emukit uses existing, potentially specialized, custom models provided by the user and wraps them into a provided interface. As such Emukit is \emph{model backend agnostic}.
...
Emukit mainly contains three high-level methods: Bayesian optimization (BO), Bayesian quadrature (BQ) and experimental design (ED) which this section briefly introduces.
...
\emph{Bayesian quadrature} \maren{cite} is a numerical method that aims to infer the integral of a black-box function (called the `integrand') given some integration measure and queries of the integrand at nodes. In contrast to Monte Carlo (MC) methods , BQ generally can accept any kind of node design and is especially sample efficient which makes it superior to MC is certain circumstances. \maren{cite} A sub-group of BQ methods are active and follow a similar decision loop as BO; the most notable difference being that acquisition functions are specific to BQ and the class of models is somewhat more restricted. Generally, active BQ methods are algorithmically similar on a high level to BO methods and can use similar modes.

Lastly, \emph{experimental design} \maren{cite} is a method that aims to collect nodes of a black-box function such that the resulting model predicts unseen function values well. \maren{say more here}

Emukit is build on the realization that all three methods (BO, BQ, ED), albeit having different numerical aims and result objects, share the same algorithmic structure: A decision-loop that computes the next node based on the current model, evaluates the user function and then updates the model accordingly. Thus Emukit contains are shared core implementation of the decision loop of all three methods which will be elaborated further on in Section~\ref{sec:lib-structure}.
...
\andrei{I am not really keen on these subsection headers. Somehow they break the flow of the section. Are they here to summarize the point in the code below? If so, how about we remove them, and just do these summaries in the text?}
