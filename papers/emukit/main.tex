...
\maren{final todo's before submitting:
  \begin{itemize}
    \item unify to UK spelling: optimiz(s)ation, minimiz(s)er, ...
    \item add citations where missing.
    \item unify macros and spelling: decision making, Emukit
    \item check if references compile correctly and uniformly (e.g., check if first names are handled correctly, ...)
    \item Section referencing does not seem to work.
  \end{itemize}
}
\andrei{thoughts:
\begin{itemize}
    \item we mostly use UK in the docs. let's roll with that \maren{sounds good}
    \item i can help to some extent with this
    \item "decision making", "Emukit". are there more? \maren{not sure, let's add them to the list above if we find more}
    \item that's weird! shall we raise this with the proceedings repo? \maren{If you have time for it, sure. There are no section numbers though which complicates things a bit.}
  \end{itemize}
}
...
%\andrei{Thanks for breaking the deadlock! One conceptual comment I have now is this. Shall we make a bit more emphasis on decision making under uncertainty? I used the phrase "decision making" a lot in the later sections.}\maren{Yes, it's included now.}
...
\emph{Experimental design} \cite{Kennedy2000, Kennedy2001, Conti2009, Conti2010} is a method that aims to collect nodes of a black-box function such that the resulting model predicts unseen function values well. \maren{say more here}
...
\maren{I find the interface capability of Emukit quite unique and a real selling point. It somehow gets lost a bit in between the lines. Shall we make a separate section called `model interfaces' or `custom backends' or sth similar with an example and simply mention here briefly that models are big interfaced?} \andrei{This is now mentioned a lot in the next section, where we show wrapper examples, and in applications where we stress that Emukit was chosen for this feature. Do you still think we need to separate this a bit more here?}\maren{I think it's good now, thanks!}
...
\andrei{I am not really keen on these subsection headers. Somehow they break the flow of the section. Are they here to summarize the point in the code below? If so, how about we remove them, and just do these summaries in the text?}\maren{We should keep the subsections. We should rather adjust the text to fit the subsections or smooth out the transitions between the subsections.}\andrei{Ok, but why? Would be great to know why you find them so helpful here, to make any sort of adjustment!}\maren{It's not related to my personal writing style. Short sections with well-defined titles are known to be easier to comprehend and parse. It's a general rule of paper writing. I know it can feel weird to have your text restructured, but trust me on this, please. The text inside the subsections can then corroborate the main points. For starters I had simply added the subsection headers where it made sense given the current text, but the text in each subsection can be adjusted or smoothed out for better reading flow if you like. I personally think it reads reasonably well as is, but feel free to do that of course!}
...
\subsection{Emukit provides basic support for sensitivity analysis}
...
\andrei{Maren, would you like to write a paragraph about your work on BQ with Alex G.? Or the one with Masha? Or both?}\maren{Yes, I can do that. Although the code of both papers unfortunately is not available online currently.} \andrei{That's quite alright, even better reason to explain what role Emukit played in them.}\maren{sounds good, added something.}
...
\maren{The shared core package may need to be a section on its own as its quite a unique feature of Emukit, perhaps including the diagram/graph that is also in the rtd. I added a placeholder subsection above. What do you think?} \andrei{I tried to run it, and bq-bo-bq does work, see a new snippet. I've added your points, slightly rephrased, there, and briefly repeated them here.}\maren{That's really cool! Thanks! A user could then also easily build an acquisition function which is equal to the sum of some normalized BO and BQ acquisition function such that both the minimiser and integral are learned equally well given the budget. So many possibilities :)}
