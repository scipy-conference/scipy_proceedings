...
.. class:: abstract
   ...
   remains live and up-to-date rather having to wait until all code has
   finished running.
...
from Markdown (and other formats) to LaTeX, HTML, Microsoft Word,
PowerPoint, and other formats. Pandoc is especially useful for documents
with embedded code that is executed during the build process. Rstudio’s
...
While using a Pandoc Markdown variant as source format brings many
...
Codebraid Preview’s preview panel can be opened using the VS Code
command palette, or by clicking the Codebraid Preview button that is
visible when a Markdown document is open. The preview panel takes the
document in its current state, converts it into HTML using Pandoc, and
displays the result using a webview. An example is shown in
Figure :ref:`screenshot`.  Since the preview is generated by Pandoc, all
Pandoc features are fully supported.
...
tracking source location. For the first time, this made it possible to
...
and creates several plots in the output, there isn’t necessarily a way
...
mapped proportionally to the vertical space occupied by the output.

Pandoc supports multi-file documents. When Pandoc is run on the command
...
documents. Once the files have been specified (for example, in a
...
.. raw:: latex
    ...
    \end{quotation}
    after processing by Codebraid and finally Pandoc.  Further examples of
...
The typical approach to executing code within Markdown documents blocks
...
recently. This design is well suited to building a document a single
time, but blocking until all code has executed is not ideal in the
context of a document preview.
...
2. As soon as any code output is available, Codebraid immediately
   ...
   executed, so that changes can be detected.

3. If the document is modified while code is running or if code output
   is received, Codebraid Preview rebuilds the document. It creates a
   ...
   conversion. The filter removes all code from the AST that is marked
   with Codebraid attributes, and replaces it with the code output that
   was stored in the AST metadata. If code has been modified since
...
chunks to it. This would bring the potential for the sort of
out-of-order execution issues that are possible in a Jupyter notebook.
Yet that would allow for much more rapid code output than is possible
otherwise, particularly in cases where large datasets must be loaded or
...
and to continue to edit the document during code execution, instead of
...
reparsing the entire document with Pandoc and then loading the resulting
HTML.
...
extension should make possible Markdown to PDF synchronization, with
