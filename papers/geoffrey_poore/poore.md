...
formats) to LaTeX, HTML, Microsoft Word, PowerPoint, and other formats.
...
While using a Pandoc Markdown variant as source format brings many advantages,
...
Codebraid Preview's preview panel can be opened using the VS Code command
...
first time, this made it possible to construct a source map for a Pandoc input
format.
...
creates several plots in the output, there isn't necessarily a way to trace
...
the vertical space occupied by the output.

Pandoc supports multi-file documents.  When Pandoc is run on the command line,
...
Once the files have been specified (for example, in a configuration file
...
The typical approach to executing code within Markdown documents blocks the
...
software, as well as by Codebraid until recently.  This design is well suited
to building a document a single time, but blocking until all code has executed
...
2. As soon as any code output is available, Codebraid immediately streams this
   ...
   includes a hash of the code that was executed, so that changes can be
   detected.

3. If the document is modified while code is running or if code output is
   received, Codebraid Preview rebuilds the document.  It creates a copy of
   ...
   conversion.  The filter removes all code from the AST that is marked with
   Codebraid attributes, and replaces it with the code output that was stored
...
execution cycle and only pass modified code chunks to it.  This would bring
the potential for the sort of out-of-order execution issues that are possible
in a Jupyter notebook.  Yet that would allow for much more rapid code output
than is possible otherwise, particularly in cases where large datasets must be
loaded or significant preprocessing is required.
...
it is possible to see code output in the preview and to continue to edit the
...
frequency.  Every update of the preview involves reparsing the entire document
with Pandoc and then loading the resulting HTML.
...
Markdown to PDF synchronization, with LaTeX as an intermediary.
