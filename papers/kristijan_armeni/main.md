...
**The research-to-newsroom gap.** Originally coined by Facebook around 2018 [@gleicher_removing_2018], the term _coordinated inauthentic behavior_ in its various definitions generally refers to a group of inauthentic users performing coordinated activity online (e.g. posting, repositing, liking etc.) with a strategic goal to mislead and manipulate public opinion [@starbird_disinformation_2019; @romero_vicente_coordinated_2024]. Computational social scientists have developed a rich body of sophisticated methods to analyze the social media datasets for insights about coordinated behavior [@mannocci_detection_2024].

Yet, the experts who research and report on CIB rarely have either the time, resources, or technical capacity to implement such methods from scratch or to execute publicly-available analysis code. In addition, many of the peer-reviewed analyses are specifically tailored to the dataset that was used in research (e.g. a Twitter dataset spanning a time-window around a specific campaign). In most cases, such analyses likely need to be refactored to be applicable to new datasets beyond the original — even if the core analyses rely on general-purpose data science methods.

**A fragmented landscape of social media data access and monitoring tools.** Social media are one of the main vehicles of public discourse and enagement. As such, social media platforms remain a target of coordinated campaigns that leverage it to sway and manipulate public opinion. A reliable access to data and tools to monitor and analyze the such online behavior are the prerequisties in order for independent researchers, data journalists, and fact-checkers to accurately report on it and exert oversight. However, the social media activity data from major platforms are commonly not publicly accessible, offer a limited access, or are locked behind paid-access APIs [@noauthor_better_2025]. Coupled with an increasingly commercialized ecosystem of social media monitoring tools [@chris_dashboards_2025], this development weakens accountability and oversight by independent researchers.

**A community-developed library of tests.** CIB Mango Tree tries to fill this gap by implementing simple, but effective methods (referred as "tests" in our parlance) into a coherent toolkit. There is no single test that can definitively determine whether a snapshot of activity on social media is coordinated and/or inauthentic. Therefore -- at its core -- the goal of CIB Mango Tree is to be a library of complementary tests, each of which leverages and highlights different aspects of potential coordination patterns (e.g. copy-pasted content, trending hashtag usage, temporal coordination etc.).
...
## Lessons learned from design constraints of civic tech

For maintainers entering civic tech from scientific computing, there are three main lessons to be learned from adopting open source practices in civic tech projects.

**Be willing to revisit technical assumptions.** Given the application and user-focused development (see {ref}`sec-community-characteristics` and @table_design), it is important to be willing to actively monitor what the needs of the target organization or community are and revise technical assumptions if needed. For example, a command-line tool (as is common in scientific computing) might be a lightweight solution for a specific application, but if civic partners prefer a graphical user interface for their needs, then that should be factored in technical decisions and roadmap.

**Leverage open source practices as social infrastructure.** While remaining flexibile about specific technical choices is important, we nevertheless recommend to implement the core practices of collaborative open source (test-driven development, version control, code reviews etc.) and leverage them for strengthening the broader community. As shared in this paper, even practices that are initially designed for bespoke engineering purposes and quality control can yield community benefits beyond developers.

**Expect greater maintainer visibility.** In our experience, the emphasized social angle coupled with small and flat teams in our civic tech community also naturally leads to a greater visibility of maintainers' work, surfacing aspects of developer relations (DevRel) roles [@oliveira_developer_2021] where the needs of maintainers and developers have to be continuously communicated with the broader team in order to move the project forward.

## Limitations

**Generalizability to different community contexts.** The experience of open source maintainership shared in this report is limited to a specific setting: a community of volunteers that meets primarily in-person with smaller teams and a relatively flat structure. This does not readily generalize to open source communities that primarily work remotely and across diverse timezones where dynamics and needs will differ. Likewise, in larger (and possibly non-volunteer) civic tech teams, the division of labor and professionalization of dedicated community roles, for example community management [@sharan_professionalising_2024] and developer relations [@oliveira_developer_2021], would be beneficial. In such setting, it would be ill-advised to expect a maintainers to also simultaneously perform community building roles.


**Effects of AI-assisted development.** Second, while we purposefully centered our report on the aspects of human-to-human collaboration in open source and civic tech, the proliferation of AI-assisted software development [e.g. @huyen_ai_2025] can not be ignored. How does the availability of code generation tools change our recommendations? It most clearly affects the consideration of onboarding friction when selecting project dependencies ({ref}`sec-dependencies`). In our experience, it pays off when choosing dependencies with a less steep learning curve even if at the expense of performance. However, the availability of code-generation tools where the user produces code through natural language input reduces the uptake friction that motivated our recommendation (for example, users can gain an understanding of the codebase by feeding it as input to generative AI for summarization etc.). In that sense, choosing dependencies with steep learning curve is perhaps no longer such a bottleneck. Yet, uncritical adoption of AI tooling can backfire. In open source communities specifically, it has led to increases in maintainer burden through unsustainable and unreviewable volume of code contributions [e.g. @afroz_ai_2026; @wolf_welcome_2026]. We thus remain cautious and advise a critical, pro-social adoption of AI tooling in civic tech by monitoring the needs of collaborators first before delegating work to AI.

## Conclusion

Design characteristics of volunteer-run projects in the Civic Tech DC reshape how successful open source maintainership looks like. Small teams with limited hierarchy, high turnover of volunteers, mix of professional interests beyond coders, and a focus on developing applications all bring to the fore the social angle of open source maintainership and practices. Maintainership effectively blends with elements of community management [@sharan_professionalising_2024]: how we adopt a certain practice depends not only on its technical merits but also on whether it facilitates participation, sustains engagement, and builds a sense of shared ownership and agency across a diverse volunteer community.
