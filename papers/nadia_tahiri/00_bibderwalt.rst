...
where :math:`n` is a number of species (or taxa), :math:`r` is a number of replicates, :math:`SA` is a size of the multiple sequence alignment (MSA), and :math:`e` is a number of refinement steps performed by the RaxML algorithm. For all :math:`SA \in {N^*}` and for all :math:`WS, S \in {N}` then :math:`nb = \left\lfloor \frac {SA - WS}{S} + 1 \right\rfloor`; where :math:`WS` is a window size, and :math:`S` is a step.
