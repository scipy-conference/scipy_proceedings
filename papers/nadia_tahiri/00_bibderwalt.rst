...
Initially, specific lineages of interest are selected from a checklist on the Dash web page. 
Subsequently, the selected lineages are utilized to query the graph database, extracting the predominant countries in which these lineages are prevalent, along with the earliest and latest recorded dates and their corresponding predominant rates. 
The obtained results are presented as an interactive Dash Table, providing an interface for applying column and row filters. This functionality allows for the exclusion of locations or lineages that are deemed irrelevant for a specific research. Additionally, predominant rates can applied to filter out samples.
